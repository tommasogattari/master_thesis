...
class PatchExpanding(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = x[:, :, :, 2::4]  # B H W C/4
        x3 = x[:, :, :, 3::4]  # B H W C/4
        x = torch.cat([torch.cat([x0, x1], 2), torch.cat([x2, x3], 2)], 1) # B 2*H 2*W C/4
        x = x.view(B, -1, C//4)  # B H/2*W/2 4*C
    ...
    def flops(self):
        H, W = self.input_resolution
        flops = (2 * H) * (2 * W) * 4 * self.dim * 2 * self.dim
        flops += H * W * self.dim // 2
        return flops
...
class PatchUnEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, embed_dim=96, out_chans=3, norm_layer=None):
        ...
        self.upconv = nn.ConvTranspose2d(embed_dim, out_chans, kernel_size=patch_size, stride=patch_size)
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.patch_unembed = PatchUnEmbed(
            img_size=img_size, patch_size=patch_size, out_chans=dim, embed_dim=dim,
            norm_layer=None)
    ...
    def forward(self, x, prev_attn=None):
        ...
        _x = x
        if self.attn_in and prev_attn is not None:
            x = self.residual_group(x, prev_attn=prev_attn)
        else:
            x = self.residual_group(x)
        if self.attn_out and type(x) == tuple:
            attn_before_softmax = x[1]
            x = x[0]
        x = self.patch_unembed(x)
        x_conv = self.conv(x)
        x = self.patch_embed(x_conv) + _x
